...
export async function getLocationByText(text) {
    ...
    const urlStr = `https://api.mapbox.com/geocoding/v5/mapbox.places/${encodeURIComponent(
        text
    )}.json?country=us&limit=5&language=en&access_token=${AppContract.mapBoxAccessToken}`;
    const response = await request(urlStr);
    if (response.success) {
        const { features } = response.data;
        const suggestions = [];
